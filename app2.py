...
with tab2:
    ...
    with col_c2:
        webrtc_streamer(key="main-cam", video_transformer_factory=VideoTransformer)
...
with tab3:
    ...
    if st.button("🎙️ 開啟語音辨識"):
        r = sr.Recognizer()
        # --- 核心優化參數 ---
        r.energy_threshold = 300       # 降低門檻，更容易抓到聲音
        r.pause_threshold = 1.2        # 延長停頓判斷，避免話沒說完就斷掉
        ...
        with sr.Microphone() as source:
            try:
                ...
                st.error("未偵測到聲音或硬體衝突。請確認麥克風已開啟。")